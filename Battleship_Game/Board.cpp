...
{
	if (userGridArray[old_x][old_y + N] == FILLED && (old_y + N) < 10) { 
		x = old_x; y = old_y + N;
		axe = RIGHT;
	}
...
{
	if (touched) {
		if (checkAdjacentCase(x, y)) { touched = false; } //IN LOC DE CHECK CASE POATE SA FIE checkComputerChoice CA SA FIE TOT RANDOM
		else { touched = false; }
	}
